                        ...
                        <div class="card">
                            ...
                            <div class="card-content">
                                <div class="card-body">
                                    ...
                                    @include('dashboard.coupons._edit_modal')

                                    @include('dashboard.includes.tostar-success')
                                    @include('dashboard.includes.tostar-error')

                                    <table id="yajra_table" class="table table-striped table-bordered"
    ...
    <script>
        $(document).ready(function() {
            ...
            var table = $('#yajra_table').DataTable({
                ...
                buttons: [{
                        extend: 'colvis',
                        text: '<i class="la la-columns"></i> ' + (lang === 'ar' ? 'الأعمدة' :
                            'Columns'),
                    ...
                    {
                        extend: 'excel',
                        text: '<i class="la la-file-excel-o"></i> ' + (lang === 'ar' ? 'إكسل' :
                            'Excel'),
                        init: function(api, node) {
                            $(node).css({
                                background: '#36b9cc',
            ...
            $(document).on('click', '.ajax_delete_btn', function(e) {
                ...
                swalWithBootstrapButtons.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('dashboard.coupons.destroy', ':id') }}".replace(
                                ':id',
                                coupon_id),
                            method: 'delete',
                            data: {
...
/* ===============================
   Update Coupon (AJAX)
================================ */
$(document).on('click', '.update_coupon_btn', function (e) {
    e.preventDefault();

    let form = $('#updateCoupon')[0];
    let formData = new FormData(form);
    let couponId = $('#edit_id').val();
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: "{{ route('dashboard.coupons.update', ':id') }}".replace(':id', couponId),
                method: "POST",
                data: formData,
                processData: false,
                contentType: false,
...
$(document).on('click', '.change_status_btn', function (e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: "{{ route('dashboard.coupons.change_status', ':id') }}".replace(':id', couponId),
                method: "POST",
                data: {
                    _token: "{{ csrf_token() }}"
                },
